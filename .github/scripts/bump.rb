...
CONTENT = File.read(ENV['CLI_VERSION_FILE'])
...
# Update CLI version
%x[sed -i '' 's/#{VERSION}/#{NEW_VERSION}/g' #{ENV['CLI_VERSION_FILE']}]
# Update App version
%x[sed -i '' 's/#{VERSION}/#{NEW_VERSION}/g' #{ENV['APP_XCCONFIG_FILE']}]
# Update version for Homebrew
%x[sed -i '' 's/#{VERSION}/#{NEW_VERSION}/g' #{ENV['HOMEBREW_CASK_FILE']}]
puts NEW_VERSION
